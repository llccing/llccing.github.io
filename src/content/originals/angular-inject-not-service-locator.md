...
sourceUrl: "http://riegler.fr/blog/2025-01-08-inject-not-service-locator"
---
...
While we can capture those dependencies in unit tests, basic unit test that just ensure construction with dependencies might miss that "hidden" dependency.
...
The same way you would write your application code, several Services, Directives and Pipes already rely on the `inject` function. This is mostly seamless to all developers.

What's interesting to look at are some functions that rely on inject. To name a few we have `toSignal`, `toObservable`, `takeUntilDestroyed`, `after(Next)Render`, `afterRenderEffect` etc.
...
If you'd like some references on the Service Locator pattern please have a look:

- [Inversion of Control Containers and the Dependency Injection pattern, on martinFowler.com](https://martinfowler.com/articles/injection.html#ServiceLocator) — This is considered the canonical reference that first properly defined the pattern
- [Service Locator is an Anti-Pattern, Mark Seemann's critical analysis](https://blog.ploeh.dk/2010/02/03/ServiceLocatorisanAnti-Pattern/) — Provides important counterpoints and explains why some consider it an anti-pattern
